...
权重：$w_j = \frac{1-e_j}{\sum_k(1-e_k)}$。最终权重：DGA组分为0.52，负载率为0.23。
...
\end{align}
引入物理约束层，将DRTA三比值法作为知识蒸馏损失项。

\subsection{问题三：RUL预测与维护优化}
退化轨迹采用改进Wiener过程建模：
$$X(t) = X(0) + \mu t + \sigma B(t)$$
RUL定义为首次穿越故障阈值的时间：$RUL = \inf\{t: X(t) >= X_\text{threshold}}$。
维护优化建模为马尔可夫决策过程(MDP)，采用Q-learning求解最优检修策略。